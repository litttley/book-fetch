...
export const generateUrls = async (fileId, pageStart, pageEnd) => {
    let urls = []
    try {
        const response = await fetch(`https://www.iiif.ku-orcas.kansai-u.ac.jp/books/${fileId}#?page=1`, {
            method: "GET",
            headers: {
                "Accept":
        ...
        const formEle = doc.querySelector(".link-manifest");
        let manifestHref = formEle.getAttribute("href");

        // console.log(manifestHref)


        const responseManifest = await fetch(manifestHref, {
            method: "GET",
            headers: {
                "Accept":
                    "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
        ...
        const manifestObj = await responseManifest?.json();
...
export const downLoadImages = async (urls, command) => {
    ...
    for (var i = 0; i < urls.length; i++) {
        try {
            ...
            const cmd = new Deno.Command(path, {
                args: [...command ,urls[i].url,`./kaFiles/${urls[i].page}.png`],
                stdout: "piped",
                stderr: "inherit",
                // cwd: "kaFiles",
            });
    ...
    try {
        await Deno.remove("./kaFiles/info.json");
    } catch (error) {
...
export const downLoaddezoomify = async () => {
    ...
    if (platform == "linux") {
        console.log("解压linux");
        // await tgz.uncompress("./dezoomify-rs-linux.tgz", "./kaFiles");
...
export const config = async () => {
    try {
        ...
        if (await checkFileExists("kaFiles/kaConfig.toml")) {
            Deno.writeTextFileSync(
                "kaFiles/kaConfig.toml",
        ...
        console.log("文件已生成:kaFiles/kaConfig.toml");
    } catch (error) {
        console.log(error);
    }
};
...
export const undownLoad = async () => {
    ...
    try {
        ...
        if (await checkFileExists("kaFiles/undownLoad.txt")) {
            const f = await Deno.open("kaFiles/undownLoad.txt", {
                read: true,
                write: true,
            });
