...
export const generateUrls = async (url, pageStart, pageEnd) => {
    let urls=[]
  try {
    const manifestObj = await getManifest(url)
...
export const downLoadImages = async (urls, command) => {
  ...
  for (var i = 0; i < urls.length; i++) {
    try {
      ...
      const cmd = new Deno.Command(path, {
        args: [...command ,urls[i].url,`./bnFiles/${urls[i].page}.png`],
        stdout: "piped",
        stderr: "inherit",
        // cwd: "bnFiles",
      });
  ...
  try {
    await Deno.remove("./bnFiles/info.json" );
  } catch (error) {
...
export const downLoaddezoomify = async () => {
  ...
  if (platform == "linux") {
    console.log("解压linux");
    // await tgz.uncompress("./dezoomify-rs-linux.tgz", "./bnFiles");
...
export const config = async () => {
  try {
    ...
    if (await checkFileExists("bnFiles/bnConfig.toml")) {
      Deno.writeTextFileSync(
        "bnFiles/bnConfig.toml",
    ...
    console.log("文件已生成:bnFiles/bnConfig.toml");
  } catch (error) {
    console.log(error);
  }
};
...
export const undownLoad = async () => {
  ...
  try {
    ...
    if (await checkFileExists("bnFiles/undownLoad.txt")) {
      const f = await Deno.open("bnFiles/undownLoad.txt", {
        read: true,
        write: true,
      });
