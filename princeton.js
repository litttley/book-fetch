  ...
  for (var i = 0; i < urls.length; i++) {
    try {
      ...
      const cmd = new Deno.Command(path, {
        args: [...command, urls[i].url, `./prFiles/${urls[i].page}`],
        stdout: "piped",
        stderr: "inherit",
        // cwd: "prFiles",
      });
...
export const downLoaddezoomify = async () => {
  ...
  if (platform == "linux") {
    console.log("解压linux");
    // await tgz.uncompress("./dezoomify-rs-linux.tgz", "./prFiles");
...
export const config = async () => {
  try {
    ...
    if (await checkFileExists("prFiles/prConfig.toml")) {
      Deno.writeTextFileSync(
        "prFiles/prConfig.toml",
    ...
    console.log("文件已生成:prFiles/prConfig.toml");
  } catch (error) {
    console.log(error);
  }
};
...
export const undownLoad = async () => {
  ...
  try {
    ...
    if (await checkFileExists("prFiles/undownLoad.txt")) {
      const f = await Deno.open("prFiles/undownLoad.txt", {
        read: true,
        write: true,
      });
