...
export const generateUrls = async (url, pageStart, pageEnd) => {
  try {
    ...
    const totalEle = doc.querySelector(".hl__viewer-link");
    const href = totalEle.getAttribute("href");
    // console.log(href)

    const urlParam = new URL(href);
    const mainfestUrl = urlParam.searchParams.get("manifestId");
    // console.log(mainfestUrl)

    if (!mainfestUrl) {
      throw new Error("获取mainfest失败");
    }
  ...
  for (var i = 0; i < urls.length; i++) {
    try {
      ...
      const cmd = new Deno.Command(path, {
        args: [...command, urls[i].url, `./harFiles/${urls[i].page}`],
        stdout: "piped",
        stderr: "inherit",
        // cwd: "harFiles",
      });
...
export const downLoaddezoomify = async () => {
  ...
  if (platform == "linux") {
    console.log("解压linux");
    // await tgz.uncompress("./dezoomify-rs-linux.tgz", "./harFiles");
...
export const config = async () => {
  try {
    ...
    if (await checkFileExists("harFiles/harConfig.toml")) {
      Deno.writeTextFileSync(
        "harFiles/harConfig.toml",
    ...
    console.log("文件已生成:harFiles/rmConfig.toml");
  } catch (error) {
    console.log(error);
  }
};
...
export const undownLoad = async () => {
  ...
  try {
    ...
    if (await checkFileExists("harFiles/undownLoad.txt")) {
      const f = await Deno.open("harFiles/undownLoad.txt", {
        read: true,
        write: true,
      });
