...
const generateUrlsStep2 = async (infoUrls) => {
  ...
  try {
    const response = await fetch(infoUrls, {
      method: "GET",
      headers: {
        "Accept":
...
export const generateUrls = async (id, pageStart, pageEnd) => {
    let urls=[]
  try {
    const response = await fetch(`https://iiif.bodleian.ox.ac.uk/iiif/manifest/${id}.json`, {
        method: "GET",
        headers: {
          "Accept":
            "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
  ...
  for (var i = 0; i < urls.length; i++) {
    try {
      ...
      const cmd = new Deno.Command(path, {
        args: [...command, urls[i].url,`./boFiles/${urls[i].page}`],
        stdout: "piped",
        stderr: "inherit",
        // cwd: "boFiles",
      });
...
export const downLoaddezoomify = async () => {
  ...
  if (platform == "linux") {
    console.log("解压linux");
    // await tgz.uncompress("./dezoomify-rs-linux.tgz", "./boFiles");
...
export const config = async () => {
  try {
    ...
    if (await checkFileExists("boFiles/boConfig.toml")) {
      Deno.writeTextFileSync(
        "boFiles/boConfig.toml",
    ...
    console.log("文件已生成:boFiles/boConfig.toml");
  } catch (error) {
    console.log(error);
  }
};
...
export const undownLoad = async () => {
  ...
  try {
    ...
    if (await checkFileExists("boFiles/undownLoad.txt")) {
      const f = await Deno.open("boFiles/undownLoad.txt", {
        read: true,
        write: true,
      });
